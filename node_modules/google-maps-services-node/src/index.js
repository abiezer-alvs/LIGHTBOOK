...
module.exports = function(config) {
    ...
    return {
            ...
            return Q.Promise(function(resolve, reject) {
                ...
                request({
                    ...
                    if (response.status!=="OK")
                        return reject(new GoogleMapsApiError(response));
                    resolve(response);
                });
        ...
        directions: function directions(origin, destination, params, cb) {
            return Q.Promise(function(resolve, reject) {

                params = defaults(params, defaultConfig.defaultParams);

                if (config.checkInputs) {
                    if (any([origin], function(origin) { return !!~origin.indexOf("|"); }))
                        throw new Error("Invalid character in origin address");
                    if (any([destination], function(destination) { return !!~destination.indexOf("|"); }))
                        throw new Error("Invalid character in destination address");
                }

                request({
                    url: config.baseUrl + "/maps/api/directions/" + params.format + "?",
                    qs: extend({ origin: origin, destination: destination }, params),
                    json: true
                }, function(err, httpIM, response) {
                    if (err)
                        return reject(err);
